...
def arc_same_as_meschini(duration):
    ...
    connections = [
        ...
        Connection(injection_rate[0], storage[1]),
        Connection(t_separation_membrane[1], storage[2]),
        ...
        Connection(detrit__storage[0], storage[3]),
        Connection(storage__cleanup[1], fuel_cleanup[0]),
        Connection(storage__cleanup[0], storage[0]),
...
def arc_series_bcrs(duration):
    ...
    # Save the main data
    output_file = os.path.join(results_dir, 'ARC_series_bcrs_results.npz')
    np.savez(output_file, **save_dict)

    # Save metadata as JSON in the same directory
    import os
    metadata_file = os.path.join(os.path.dirname(output_file), 'ARC_series_bcrs_metadata.json')
    with open(metadata_file, 'w') as f:
        json.dump(scope_metadata, f, indent=2)
